...
#%%
#Might have to change parents because i did this in interactive mode with jupyter extension
...
df_cat_b = df_cat_b.drop(columns=['category'])
#format date
...
#rename all columns to have _cat_b suffix
...
df = df.merge(df_cat_b[['date','quota_cat_b','quota_premium_cat_b','total_bids_received_cat_b']], on='date', how='left')
...
#keep only date, quota per bid
quota_months = quota_months[['date', 'quota_per_bid']]
...
df = df.merge(quota_biweekly[['quota_per_bid']], left_index=True, right_index=True, how='left')
...
rb = registrations_biweekly[["date","regis_cat_a", "regis_cat_b"]].reset_index(drop=True)
rb.index = rb.index + 2   # start filling from 3rd row (index 2)

df = (
    df.reset_index(drop=True)
      .merge(rb[["regis_cat_a", "regis_cat_b"]], left_index=True, right_index=True, how="outer")
      .sort_index()
...
tpb = total_population_biweekly[["total_cars_cat_a", "total_cars_cat_b"]].reset_index(drop=True)
tpb.index = tpb.index + 2   # start filling from 3rd row (index 2)

df = (
    df.reset_index(drop=True)
      .merge(tpb[["total_cars_cat_a", "total_cars_cat_b"]], left_index=True, right_index=True, how="outer")
      .sort_index()
      .reset_index(drop=True)
)
# %%
...
df = df.merge(pqp_biweekly[['pqp_cat_a', 'pqp_cat_b']], left_index=True, right_index=True, how='left')
# %%
#clean cpi data
...
df = (
    df.reset_index(drop=True)
      .merge(cpi_biweekly[['cpi_all', 'cpi_transport','cpi_private_transport','cpi_cars','cpi_transport_accessories','cpi_petrol','cpi_lubricants','cpi_maintenance','cpi_transport_others','cpi_transport_insurance','cpi_land_transport','cpi_bus_train_fare','cpi_ptp_transport_services','cpi_commuting_fares','cpi_other_land_transport_services']], left_index=True, right_index=True, how="outer")
      .sort_index()
      .reset_index(drop=True)
)
